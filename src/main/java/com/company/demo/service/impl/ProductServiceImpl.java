...
public class ProductServiceImpl implements ProductService {
  ...
  public List<ProductInfoDto> getListSuggestProduct() {
    ...
    if (configs.size() > 0) {
      ...
      List<ProductInfoDto> products = productRepository
          .getListSuggestProduct(config.getOboChoices(), 5);
  ...
  public PageableDto filterProduct(FilterProductReq req) {
    ...
    if (req.getSizes().size() > 0) {
      products = productRepository
          .searchProductBySize(req.getBrands(), req.getCategories(), req.getMinPrice(),
              req.getMaxPrice(), req.getSizes(), limit, page.calculateOffset());

      totalItems = productRepository
          .countProductBySize(req.getBrands(), req.getCategories(), req.getMinPrice(),
              req.getMaxPrice(), req.getSizes());
    } else {
      products = productRepository
          .searchProductAllSize(req.getBrands(), req.getCategories(), req.getMinPrice(),
              req.getMaxPrice(), limit, page.calculateOffset());
      totalItems = productRepository
          .countProductAllSize(req.getBrands(), req.getCategories(), req.getMinPrice(),
              req.getMaxPrice());
    }
    ...
    PageableDto result = new PageableDto(promotionService.checkPublicPromotion(products),
        totalPages, req.getPage());
  ...
  public PageableDto searchProductByKeyword(String keyword, Integer page) {
    ...
    // Get list product and totalItems
    List<ProductInfoDto> products = productRepository
        .searchProductByKeyword(keyword, limit, pageInfo.calculateOffset());
  ...
  @Override
  public PageableDto adminGetListProduct(String id, String name, String category, String brand,
      String order, String direction, int page) {
    int limit = 15;
    PageUtil pageInfo = new PageUtil(limit, page);

    List<Product> products = productRepository
        .adminGetListProduct(id, name, category, brand, order, direction, limit,
